...
export function TeachersCoursesList({
  ...
  const { data, error, isLoading, requestFunc } =
    useFetchWithToken<Course[]>(endpoint);
  ...
  if (userRole?.toLowerCase() !== 'teacher')
    return (
      <p className={styles.message}>
        You are not authorized to view teacher courses.
      </p>
    );
  ...
  if (!data || data.length === 0)
    return <p className={styles.message}>No courses found for this teacher.</p>;
